...
public final class RunOpenKyotoScenario {
	...
	public static void main(String[] args) {
		...
		if (args == null || args.length == 0 || args[0] == null) {
			System.out.println("Running OpenKyotoScenario with default configuration.");
			MATSimApplication.runWithDefaults(OpenKyotoScenario.class, args);
			return;
		} else {
			// Check if args[0] is -h or --help
			...
			// Parse the hyperparameter values and store them in thetaMap
			parseTheta(args[0]);

			// Store the simulation number
			simulation_number = args[1];

			// Load the configuration file (if provided) or use the default one
			...
			// Set the output directory (if provided) or use the default one
		...
		// We set the random seed truly randomly since our previous study showed that setting it to a fixed value does not help
		// to reproduce the results. Better to assume complete stochasticity then. - OB
		...
		// Logging
		// We log only at the end of the simulation to save compute time - OB
		...
		// Load the scenario with modified config and launch the simulation
